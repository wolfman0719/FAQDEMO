    ...
    <table style={{ width: "100%" }}><tbody>

	  {
        reftopics.map((reftopic: any, index: number) => (
		  ...
		  <td style={{ padding: 0, verticalAlign: "middle" }}><button style={{ width: "100%", textAlign: "left", color: "#000000", padding: "8px 16px", display: "block" }} className="btn-flat waves-effect topic-item-btn" onClick={() => onClickItem(reftopic.split(comma)[0])}>{reftopic.split(comma)[1]}</button></td>
		  </tr>
    	)
    )}
	</tbody></table>
